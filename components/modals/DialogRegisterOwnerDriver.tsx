...
import { CommandEmpty, CommandGroup, CommandInput, CommandItem, CommandList } from "../ui/commandCustom";
...
export function DialogRegisterOwnerDriver({ }: Props) {
    ...
    console.log("name", path.split("/")[2]);

    const { apiListCity } = apiAddress()
    const [openCombobox, setOpenCombobox] = useState<boolean>(false);
    const [dataCity, setDataCity] = useState<any>([]);
    const { openDialogRegisterOwnerDriver, setOpenDialogRegisterOwnerDriver } = useDialogRegisterOwnerDriver();
    ...
    return (
        <>
            <Dialog modal={true} open={openDialogRegisterOwnerDriver} >
                {/* <DialogOverlay /> */}
                {openDialogRegisterOwnerDriver && (
                    <div
                        onClick={() => setOpenDialogRegisterOwnerDriver(false)}
                                                ...
                                                <FormControl>
                                                    <>
                                                        <Popover open={openCombobox} onOpenChange={() => setOpenCombobox(!openCombobox)}   >
                                                            ...
                                                            <PopoverContent className="lg:max-w-[700px] lg:w-[700px] max-w-[95%] w-[95%] p-0">
                                                                <Command className="w-full">
                                                                    <div className="relative">
                                                                        <Input
                                                                            className="focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0 pl-8 text-sm"
                                                                            onChange={({ target: { value } }) => fetchListCity(value)}
