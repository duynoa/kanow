...
const ListCarsDriver = (props: Props) => {
    ...
    useEffect(() => {
        const handleWheel = () => {
            ...
            if (lastScrollCurrentRef) {
                ...
                if (currentScroll >= lastRefBottom - ALLOWED_OFFSET && !isAtBottomRef.current && isStateListCarsDriver.isLoadingScroll === false) {
                    // Bạn đã cuộn đến cuối phần ScrollArea
                    if (isStateListCarsDriver.listCardCars && isStateListCarsDriver.next !== null) {
                        queryKeyIsStateListCarsDriver({ isLoadingScroll: true });
                        ...
                        const fetchDataListCar = async () => {
                            ...
                            if (data && data?.links && data?.data && data?.base) {
                                ...
                                const lastElementIndex = isStateListCarsDriver.listCardCars.length - 1;
                                // Lấy id của phần tử đầu tiên trong mảng mới
                                const lastElementId = isStateListCarsDriver.listCardCars && isStateListCarsDriver.listCardCars.length > 0 ? `card-${isStateListCarsDriver.listCardCars[lastElementIndex]?.id}` : "";
                                ...
                                queryKeyIsStateListCarsDriver({
                                    listCardCars: isStateListCarsDriver.listCardCars,
                                    next: data?.links?.next,
                                    page: data?.links?.next !== null ? isStateListCarsDriver.page + 1 : isStateListCarsDriver.page,
                                    isLoadingScroll: false,
                                });
                            }
                        };
        ...
        scrollContainerRef,
        isStateListCarsDriver.next,
        isStateListCarsDriver.listCardCars,
        isStateListCarsDriver.page,
        isStateListCarsDriver.isLoadingScroll,
        isVisibleMobile
    ]);
        ...
        if (car_id && index !== undefined) {
            ...
            try {
                ...
                if (data.result && getCookie !== "kanow" && getCookie !== undefined) {
                    let newDataTest = isStateListCarsDriver?.listCardCars.map((item: any) => {
                        if (item.id === car_id) {
                            return {
                                ...item,
    ...
    const handleFilterClick = async (item: any, type?: string) => {
        if (item?.type === "star_search") {
            const newStarSearch = isStateListCarsDriver?.dataParams?.star_search !== item.value ? item.value : 0;
            ...
            const newTramSearch = isStateListCarsDriver?.dataParams?.tram_search !== item.value ? item.value : 0;
            ...
            const newDiscountSearch = isStateListCarsDriver?.dataParams?.discount_search !== item.value ? item.value : 0;
            ...
            const newBookCarFlashSearch = isStateListCarsDriver?.dataParams?.book_car_flash !== item.value ? item.value : 0;
            ...
            const newMortgageSearch = isStateListCarsDriver?.dataParams?.mortgage !== item.value ? item.value : 0;
            ...
            const newDeliveryCarSearch = isStateListCarsDriver?.dataParams?.delivery_car !== item.value ? item.value : 0;
                ...
                <div className='custom-container grid xxl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 3xl:gap-6 gap-4 justify-start h-full'>
                    {
                        isStateListCarsDriver.onSuccess.onSuccessPage ?
                            ...
                            :
                            (
                                isStateListCarsDriver?.listCardCars && isStateListCarsDriver?.listCardCars?.length > 0 ?
                                    isStateListCarsDriver?.listCardCars?.map((card: any, index: number) => (
                                        <Link
                                            id={`card-${card.id}`}
                                            key={card.id}
                                            ...
                                            href={`/detail-car/${card.id}?type=2&${ConvertToSlug(card?.name_car)}`}
                                            prefetch={false}
                                        >
                                            {
                                    ...
                                    :
                                    <Nodata type="list-cars" className="xxl:col-span-4 lg:col-span-3 md:col-span-2 col-span-1" />
                            )
