...
const PaymentCar = ({}: Props) => {
  ...
  const { dateReal, dateTemp, numberDay, validateDateSubmit, dataCalendar, setOpenDialogCalendar, setTypeCarCalendar } =
    useDialogCalendar();

  const { setOpenDialogPromotion } = useDialogPromotion();
  const { setOpenDialogPromotion: setOpenDialogViettelPayPromotion } = useDialogViettelPayPromotion();
  ...
  const handleOpenDialog = async (type: string, typeTime?: string) => {
    ...
    } else if (type === 'viettelpay_promotion') {
      setOpenDialogViettelPayPromotion(true);
    ...
    if (isStateDetailCar?.dataDetailCar?.promotion?.length > 0) {
      queryKeyIsStateDetailCar({
        price: {
          ...
          total_amount: isStateDetailCar?.price?.temp_total_amount - isStateDetailCar?.dataDetailCar?.promotion[0]?.price_promotion,
          price_depoist:
            (isStateDetailCar?.price?.temp_total_amount - isStateDetailCar?.dataDetailCar?.promotion[0]?.price_promotion) *
            (isStateDetailCar?.price?.percent_deposit / 100),
          cash_on_delivery:
            isStateDetailCar?.price?.temp_total_amount -
            isStateDetailCar?.dataDetailCar?.promotion[0]?.price_promotion -
            (isStateDetailCar?.price?.temp_total_amount - isStateDetailCar?.dataDetailCar?.promotion[0]?.price_promotion) *
              (isStateDetailCar?.price?.percent_deposit / 100),
        },
        infoPromotion: {
      ...
      queryKeyIsStateDetailCar({
        price: {
          ...
          cash_on_delivery:
            isStateDetailCar?.price?.temp_total_amount -
            isStateDetailCar?.price?.temp_total_amount * (isStateDetailCar?.price?.percent_deposit / 100),
        },
        infoPromotion: {
          selectPromotion: '0',
          activePromotion: null,
  ...
  const handleSubmitCar = () => {
    ...
    } else if (informationUser?.id === isStateDetailCar?.dataDetailCar?.customer?.id && getCookie !== 'kanow' && getCookie !== undefined) {
      toastCore.error('Chủ xe không thể đặt xe của chính mình!');
    } else if (
      (informationUser?.drivingLiscense?.status == 0 || informationUser?.drivingLiscense?.status == 2) &&
      getCookie !== 'kanow' &&
      getCookie !== undefined
    ) {
    ...
    <div className="flex flex-col 3xl:gap-4 lg:gap-2 gap-4 xxl:w-[30%] xxl:max-w-[30%] lg:w-[35%] lg:max-w-[35%] w-full max-w-full h-full lg:order-none order-1">
      <div className="flex flex-col gap-2 xl:px-6 xl:py-4 p-4 bg-[#C2F9F9]/[63] rounded-2xl ">
        <div className="flex flex-row items-center gap-2">
          <div className="3xl:text-2xl text-xl text-[#16171B] font-semibold">Bảo hiểm thuê xe</div>
          <PiShieldCheckFill onClick={() => console.log('check')} className="text-[#3561FF] 3xl:text-2xl text-xl cursor-pointer" />
        </div>
      ...
      <div className="flex flex-col 3xl:gap-6 gap-4 xl:p-6 p-4 bg-white border rounded-2xl">
        <div className="flex items-center gap-1 flex-1">
          {isStateDetailCar?.dataDetailCar?.promotion?.length > 0 ? (
            <div className="3xl:text-4xl md:text-3xl text-2xl text-[#D7D9E0] font-medium line-through">
              {isStateDetailCar?.price?.price_before_promotion
                ? FormatNumberToThousands(isStateDetailCar?.price?.price_before_promotion)
                : ''}
            </div>
          ) : null}
          <div className="flex">
            <span className="3xl:text-4xl md:text-3xl text-2xl text-[#1AC5CA] font-bold">
              {isStateDetailCar?.price?.price_after_promotion
                ? FormatNumberToThousands(isStateDetailCar?.price?.price_after_promotion)
                : ''}
            </span>
            <span className="3xl:text-base md:text-sm text-xs text-[#585F71] flex justify-start font-semibold capitalize">/ngày</span>
          </div>
        </div>

        <div className="flex flex-col gap-2">
          <div className="3xl:text-base text-sm text-[#16171B] font-semibold">Nhu cầu thuê xe</div>
          ...
          <div>
            {dateTemp ? (
              <Button
                ...
                className={cn(
                  `${
                    validateDateSubmit ? 'border border-red-500' : 'border-0 '
                  }  px-4 py-3 w-full justify-start text-left font-normal rounded-xl bg-[#F6F6F8]/70  3xl:text-base 2xl:text-sm xl:text-[13px] lg:text-xs md:text-sm text-xs`,
        ...
        {typeCarDetail == '2' ? (
          <div className="flex flex-col gap-2 bg-[#2FB9BD]/10 p-4 rounded-lg">
            <div className="3xl:text-base text-sm text-[#16171B] font-semibold">Lộ trình</div>

            <div className="flex flex-row items-center gap-1">
              <div className="3xl:text-sm text-xs text-[#767676]">Di chuyển liên tỉnh, trả khách tại điểm đón.</div>
              {isVisibleTablet ? (
            ...
            <div className="flex flex-col gap-4">
              ...
              {isStateDetailCar.map.coordinates.latTo != 0 && isStateDetailCar.map.coordinates.lngTo != 0
                ? isStateDetailCar.map.valueAddressDestination &&
                  isStateDetailCar.map.valueAddressDestination?.map((destination, index) => (
                ...
                : valueAddressDestination &&
                  valueAddressDestination?.map((destination, index) => (
                    <div className="flex flex-col gap-2 w-full" key={index}>
                      <Label className="text-sm text-[#6F7689]" htmlFor="place">
            ...
            <div className="3xl:text-base text-sm text-[#16171B] font-semibold">Thông tin lộ trình</div>

            <div className="flex flex-col gap-2">
              <div className="flex flex-row items-center justify-between">
                <div className="3xl:text-base text-sm text-[#16171B] font-thin">Tổng lộ trình</div>
              ...
              <div className="flex flex-row items-center justify-between">
                <div className="flex flex-row items-center gap-1">
                  <div className="3xl:text-base text-sm text-[#16171B] font-thin">Số km được đi</div>
                  {isVisibleTablet ? (
                        ...
                        <div className="max-w-[320px]">
                          <span
                            dangerouslySetInnerHTML={{
                              __html: `${
                                isStatePolicy?.dataPolicy?.car_talent?.total_km_car_talent
                                  ? isStatePolicy?.dataPolicy?.car_talent?.total_km_car_talent
                                  : ''
                              }`,
                            }}
                ...
                <>
                  {isStateDetailCar?.dataDetailCar?.surcharge_car?.map((item) => (
                    <div key={`id-${item.id}`} className="flex flex-row items-center justify-between">
                      <div className="3xl:text-base text-sm text-[#16171B] font-thin">{item.name ? item.name : ''}</div>
                      <div className="3xl:text-base text-sm text-[#3E424E] font-semibold">
        ...
        <div className="flex flex-col gap-2">
          <div className="flex flex-col gap-2 pb-3 border-b">
            <div className="flex justify-between items-center">
              <div className="flex flex-row items-center gap-2">
                <div className="3xl:text-base text-sm text-[#3E424E]">Đơn giá thuê</div>
                {isVisibleTablet ? (
                  <div onClick={() => setOpenDialogAnswerPolicy(true, 'setting_price_car')}>
                  ...
                  <ActionTooltip
                    ...
                    label={
                      <div className="2xl:max-w-[560px] xl:max-w-[520px] max-w-[420px]">
                        <span
                          dangerouslySetInnerHTML={{
                            __html: `${isStatePolicy?.dataPolicy?.setting_price_car ? isStatePolicy?.dataPolicy?.setting_price_car : ''}`,
                          }}
                        />
                      </div>
                    }
              ...
              <div className="text-[#3E424E] font-semibold 3xl:text-base text-sm">
                {FormatNumberDot(isStateDetailCar?.price?.rent_cost_day ? isStateDetailCar?.price?.rent_cost_day : 0)}
                <span>đ/ngày</span>
              </div>
            </div>
            ...
            <div className="flex justify-between items-center">
              <div className="flex flex-row items-center gap-2">
                <div className="3xl:text-base text-sm text-[#3E424E]">Bảo hiểm thuê xe</div>
                {isVisibleTablet ? (
                    ...
                    label={
                      <div className="2xl:max-w-[560px] xl:max-w-[520px] max-w-[420px]">
                        <span
                          dangerouslySetInnerHTML={{
                            __html: `${
                              isStatePolicy?.dataPolicy?.setting_insurance_car ? isStatePolicy?.dataPolicy?.setting_insurance_car : ''
                            }`,
                          }}
                        />
                      </div>
          ...
          <div className="flex justify-between items-center">
            <div className="3xl:text-base text-sm text-[#3E424E] font-medium">Tổng tạm tính</div>
            <div className="text-[#3E424E] font-semibold 3xl:text-base text-sm">
              {FormatNumberDot(isStateDetailCar?.price?.temp_total_amount)}
              {numberDay && numberDay === 1 ? <span>đ/ngày</span> : <span>đ/{numberDay} ngày</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2 bg-[#F9F9FA] rounded-xl p-4">
            <div className="3xl:text-lg xl:text-base text-sm text-[#2C2F31] font-semibold">Khuyến mãi</div>

            <RadioGroup className="flex flex-col gap-3" value={isStateDetailCar?.infoPromotion?.selectPromotion}>
              {isStateDetailCar?.dataDetailCar?.promotion?.length > 0 ? (
              ...
              <div className="flex flex-col gap-y-2 caret-transparent">
                {isStateDetailCar?.infoPromotion?.activePromotion && isStateDetailCar?.infoPromotion?.selectPromotion === '1' ? (
                    ...
                    <Label className="flex flex-row items-center justify-between gap-2 w-full cursor-pointer">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <div className="w-[90%] max-w-[90%] flex items-center gap-2">
                            <div className="xl:text-base text-sm">
                              ...
                              <span className="font-semibold uppercase">{isStateDetailCar?.infoPromotion?.activePromotion?.code}</span>
                            </div>
                      ...
                      {isStateDetailCar?.infoPromotion?.activePromotion?.percent !== 0 ? (
                        ...
                        <div className="3xl:text-base xl:text-sm text-xs text-[#2FB9BD] font-semibold">
                          -
                          {FormatNumberDot(
                            isStateDetailCar?.infoPromotion?.activePromotion?.cash
                              ? isStateDetailCar?.infoPromotion?.activePromotion?.cash
                              : 0
                          )}
                          đ
                ...
                {isStateDetailCar?.infoPromotion?.activePromotion && isStateDetailCar?.infoPromotion?.selectPromotion === '2' ? (
                  <div className="flex items-center gap-2">
                    <RadioGroupItem
                      value={isStateDetailCar?.infoPromotion?.selectPromotion}
                      checked={isStateDetailCar?.infoPromotion?.selectPromotion === '2' ? true : false}
                      className="w-5 h-5 border-[#D7D9E0] data-[state=checked]:text-[#2FB9BD] data-[state=checked]:border-[#2FB9BD]"
                    />

                    <Label className="flex flex-row items-center justify-between gap-2 w-full max-w-full cursor-pointer">
                      <div className="flex items-center gap-1 flex-1">
                        <Image
                          src="/icon/payment/logo-viettelpay.svg"
                          alt="viettelpay"
                          width={80}
                          height={80}
                          className="w-5 max-w-5 h-5 object-contain fill-[#2FB9BD]"
                        />
                        <div className="flex items-center 3xl:gap-2 gap-1">
                          <div className="flex gap-1 xl:text-base text-sm">
                            <span className="font-normal">Mã</span>{' '}
                            <span className="font-semibold uppercas truncate 3xl:max-w-[150px] xl:max-w-[115px] lg:max-w-[80px] sm:max-w-full max-w-[170px] max-[425px]:max-w-[80px]">
                              {isStateDetailCar?.infoPromotion?.activePromotion?.code}
                            </span>
                          </div>
                          <div onClick={(event) => handleRemoveDiscount(event)}>
                            <FaDeleteLeft className="size-5 text-rose-500" />
                  ...
                  <div className="flex items-center gap-2">
                    <RadioGroupItem
                      ...
                      value={isStateDetailCar?.infoPromotion?.selectPromotion}
                      checked={isStateDetailCar?.infoPromotion?.selectPromotion === '2' ? true : false}
                      onClick={() => handleOpenDialog('viettelpay_promotion')}
                      className="w-5 h-5 border-[#D7D9E0] data-[state=checked]:text-[#2FB9BD] data-[state=checked]:border-[#2FB9BD]"
                    />
                    <Label htmlFor="2" className="flex flex-row items-center justify-between gap-2 w-full cursor-pointer">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          <Image
                            src="/icon/payment/logo-viettelpay.svg"
                            ...
                            className="w-5 max-w-5 h-5 object-contain fill-[#2FB9BD]"
                          />
                          <div className="w-[90%] max-w-[90%] 3xl:text-lg xl:text-base text-sm ">Giảm giá ViettelPay</div>
                        </div>
                      </div>
                      <TiArrowSortedUp className="3xl:text-2xl text-xl text-[#16171B] rotate-90" />
                    </Label>
          ...
          <div className="flex justify-between items-center">
            <div className="3xl:text-base text-sm text-[#3E424E] font-medium">Thành tiền</div>
            <div className="text-[#3E424E] font-semibold 3xl:text-base text-sm">
              {FormatNumberDot(isStateDetailCar?.price?.total_amount ? isStateDetailCar?.price?.total_amount : 0)}
              {numberDay === 1 ? <span>đ/ngày</span> : <span>đ/{numberDay} ngày</span>}
            </div>
          </div>
        </div>
      ...
      <div className="flex w-full items-center justify-center">
        {getCookie !== 'kanow' && getCookie !== undefined ? (
          <div
            onClick={() => setOpenDialogReportCar(true)}
            className="3xl:text-base text-sm text-[#FA3434] hover:text-[#FA3434]/80 duration-300 transition-all font-semibold cursor-pointer w-fit text-center caret-transparent"
          >
            Báo cáo xe này
          </div>
        ) : (
