...
const MyCar = ({ }: Props) => {
    ...
    const { apiTextSearch } = useGoogleApi()
                ...
                const fetchLocationAddress = async () => {
                    ...
                    if (data && data.code === "ok") {
                        const dataJson = {
                            ...
                            lng: data.result[0].location.lng,
                            latTo: data.result[1].location.lat,
                            lngTo: data.result[1].location.lng,
                            latCurrent: 0,
                            lngCurrent: 0,
                        }
        ...
        <>
            {
                isStateProfileMyCar.isLoadingCar ?
                    ...
                    :
                    isStateProfileMyCar.dataMyCar?.length > 0 ?
                        isStateProfileMyCar.dataMyCar.map((e, index) => {
                            const typeCustomer = !Array.isArray(e.car) ? 1 : 2
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className='3xl:w-[20%] xxl:w-[20%] 2xl:w-[20%] xl:w-[20%] lg:w-[15%] w-full flex lg:flex-col flex-row justify-center items-center gap-4 my-auto'>
                                        {/* <Link
                                            href={`/vehicle-management-mobile/menu-mobile?key=${e.id}&t=0`}
                                ...
                                2xl:py-3 xl:py-2.5 lg:py-1.5 py-2.5  rounded-xl cursor-pointer hover:scale-105  uppercase transition-all overflow-hidden  lg:border border-[1.5px] uppercases`}
                                        ...
                                        <Link
                                            href={`/vehicle-management/information?key=${e.id}&t=0`}
                                            className={`bg-[#2FB9BD]/80 hover:bg-[#2FB9BD]/80 text-white border-[#2FB9BD] w-full text-center xl:text-sm lg:text-[9px] text-sm
                                2xl:py-3 xl:py-2.5 lg:py-1.5 py-2.5  rounded-xl cursor-pointer hover:scale-105  uppercase transition-all overflow-hidden  lg:border border-[1.5px] uppercases`}
                                        >
                                            Quản lý xe
                                        </Link>
                                        <Link
                                            onClick={() => handleClickDetailCar(e, typeCustomer)}
                                            href={`/detail-car/${e.id}?type=${typeCustomer}&${ConvertToSlug(e?.name_car)}`}
                                            className={` hover:bg-[#2FB9BD]/80 text-[#2FB9BD] hover:text-white border-[#2FB9BD] w-full text-center xl:text-sm lg:text-[9px] text-sm 
                                 2xl:py-3 xl:py-2.5 lg:py-1.5 py-2.5  rounded-xl cursor-pointer hover:scale-105  uppercase transition-all overflow-hidden  lg:border border-[1.5px] uppercases`}
                                        >
                                            Xem chi tiết
                                        </Link>
