...
const MyTripTalentedCar = ({ isState }: Props) => {
    ...
    return <>
        {isState.isLoadingCar ?
            ...
            :
            isState.dataMyTripsTalented?.length > 0 ? isState.dataMyTripsTalented.map((e: IArrayMyTripCar, index: number) => {
                return <React.Fragment key={e.id}>
                    {isVisibleMobile ?
                        <Link
                            href={`/info-rental-car/${e.id}?type=${isState.tab}`}
                            id={`card-${e.id}`}
                            className={`flex flex-wrap gap-2 group bg-white border-[#D7D9E0] ${index === isState.dataMyTripsTalented?.length - 1 ? 'border-b-0' : 'border-b'} pb-4 w-full relative z-0`}
                        >
                            <div
